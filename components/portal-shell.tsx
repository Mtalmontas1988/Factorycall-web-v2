...
import { Add, Brightness4Outlined, Close, DashboardOutlined, ErrorOutline, FilterListOutlined, LogoutOutlined, Menu, NotificationsNoneOutlined, Search, SortOutlined } from '@mui/icons-material';
import { Alert, AppBar, Avatar, Badge, Box, Button, Chip, CssBaseline, DialogActions, DialogContent, DialogTitle, Drawer, FormControl, IconButton, InputAdornment, List, ListItemButton, ListItemIcon, ListItemText, Menu as MuiMenu, MenuItem, Paper, Select, Skeleton, Stack, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, TextField, ThemeProvider, Toolbar, Tooltip, Typography, useMediaQuery } from '@mui/material';
...
import { PhotoStandardsModule } from './photo-standards-module';
...
const sidebarGroups = [
    { label: 'SERVISAS', slugs: ['is-kvietimai', 'gyvi-is-kvietimai', 'istorija', 'statistika', 'ataskaitos', 'irenginiai', 'nuotrauku-standartai', 'sandelys', 'prevenciniai-darbai', 'ai'] },
    { label: 'VALDYMAS', slugs: ['technikai', 'operatoriai', 'imones', 'linijos', 'gedimai'] },
    { label: 'ADMIN', slugs: ['vartotojai', 'administratoriai', 'roles', 'fcm-diagnostika', 'pranesimai', 'nustatymai'] },
];
const navigationKey = (slug: string) => ({
    ...
    'nuotrauku-standartai': 'photoStandards.title',
...
function Sidebar({ close, expanded, onExpand }: {
    ...
    return (
        ...
        >
            <Box sx={{ minHeight: 76, px: 1.25, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {expanded ? <BrandLogo width={190} /> : <BrandLogo compact />}
                {expanded && <IconButton onClick={close} sx={{ display: { md: 'none' }, position: 'absolute', right: 10 }}><Close /></IconButton>}
            </Box>
            <Box
                tabIndex={0}
                sx={{
                    ...
                    '&::-webkit-scrollbar-thumb': { bgcolor: 'transparent', borderRadius: 99 },
                    '&:hover, &:focus-visible': { scrollbarColor: '#40506a transparent' },
                    '&:hover::-webkit-scrollbar-thumb, &:focus-visible::-webkit-scrollbar-thumb': { bgcolor: '#40506a' },
                }}
            >
                <List dense disablePadding sx={{ mb: 1 }}>
                    <Tooltip title={expanded ? '' : t('navigation.dashboard')} placement="right">
                        <ListItemButton onClick={() => router.push('/')} selected={pathname === '/'} sx={{ minHeight: 48, justifyContent: expanded ? 'initial' : 'center', borderRadius: 2, mb: 1, '&.Mui-selected': { bgcolor: '#254b78' } }}>
                ...
                {sidebarGroups.map(group => {
                    const groupModules = modules.filter(item => group.slugs.includes(item.slug));
                    return <Box key={group.label} sx={{ mb: 1.25 }}>
                        {expanded && <Typography variant="overline" sx={{ display: 'block', px: 1.25, pt: 1, pb: .75, color: 'text.secondary', fontWeight: 800, fontSize: 10, letterSpacing: 1.25 }}>{group.label}</Typography>}
                        <List dense disablePadding>
                            {groupModules.map(item => {
                                ...
                                return <Tooltip title={expanded ? '' : t(navigationKey(item.slug))} placement="right" key={item.slug}>
                                    <ListItemButton onClick={() => router.push(`/${item.slug}`)} selected={pathname === `/${item.slug}`} sx={{ minHeight: 48, justifyContent: expanded ? 'initial' : 'center', borderRadius: 2, color: 'text.secondary', mb: 1, '&.Mui-selected': { bgcolor: '#254b78', color: 'text.primary' }, '&:hover': { bgcolor: '#202938', color: 'text.primary' } }}>
...
function ModulePage({ module, openDialog }: {
    ...
    if (module.slug === 'nuotrauku-standartai') return <PhotoStandardsModule />;
...
export function PortalShell({ slug, data }: {
    ...
    const router = useRouter();
    useEffect(() => {
        const onGlobalSearch = (event: KeyboardEvent) => {
            if (event.key !== 'Enter' || !(event.target instanceof HTMLInputElement) || event.target.placeholder !== 'Ieškoti portale...') return;
            const query = event.target.value.trim();
            if (!query) return;
            event.preventDefault();
            router.push(`/gyvi-is-kvietimai?query=${encodeURIComponent(query)}`);
        };
        document.addEventListener('keydown', onGlobalSearch);
        return () => document.removeEventListener('keydown', onGlobalSearch);
    }, [router]);
    ...
    const portalUser = users.find(item => item.id === userId || item.uid === userId || item.email?.toLocaleLowerCase() === authUser?.email?.toLocaleLowerCase());
    const headerName = portalUser?.name?.trim() || authUser?.displayName?.trim() || authUser?.email || '—';
    const headerEmail = portalUser?.email?.trim() || authUser?.email || '—';
    const headerRole = portalUser?.role?.trim() || '—';
    const avatarLabel = headerName.split(/\s+/).filter(Boolean).slice(0, 2).map(part => part[0]).join('').toUpperCase() || '?';
    const logout = async () => {
    ...
    const { operators, error: operatorsError, loading: operatorsLoading } = useOperators(needsOperators, initialOperators);
    const { items: preventiveWorks, error: preventiveWorksError, loading: preventiveWorksLoading } = usePreventiveWorks(slug === 'ataskaitos');
    const { items: tokens, error: tokensError, loading: tokensLoading } = useTokens(slug === 'fcm-diagnostika');
