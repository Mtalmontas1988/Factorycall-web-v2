...
export function ReportsModule({ calls, technicians, operators, preventiveWorks }: Props) {
  ...
  const report = useMemo<ReportDocument>(() => {
    ...
    const rows = (start ? filteredCalls.filter(call => Number(call.createdTime ?? 0) >= start) : filteredCalls).map(call => ({ ID: call.callNumber || '—', Įmonė: call.company || companyName, Linija: call.line || '—', Gedimas: call.problem || call.title || '—', Prioritetas: call.priority || '—', Būsena: call.status || '—', Technikas: call.technician || '—', Sukurta: formatDateTime(call.createdTime ?? call.date, locale) }));
    return { ...base, columns: ['ID', 'Įmonė', 'Linija', 'Gedimas', 'Prioritetas', 'Būsena', 'Technikas', 'Sukurta'].map(label => ({ label, value: '' })), rows };
  }, [calls, companies, companyId, kind, locale, operators.length, preventiveWorks.length, t, technicians]);
  const names: Record<ReportKind, string> = { fault: t('reports.fault'), technicians: t('reports.technicians'), kpi: t('reports.kpi'), daily: t('reports.daily'), monthly: t('reports.monthly') };
  return <Stack spacing={2.5}><Box><Typography variant="h4">{t('reports.title')}</Typography><Typography color="text.secondary" mt={0.5}>{t('reports.subtitle')}</Typography></Box><Paper sx={{ p: 2.5, border: '1px solid #293342' }}><Stack direction={{ xs: 'column', md: 'row' }} spacing={2}><FormControl fullWidth><InputLabel>{t('reports.type')}</InputLabel><Select value={kind} label={t('reports.type')} onChange={event => setKind(event.target.value as ReportKind)}>{Object.entries(names).map(([value, label]) => <MenuItem key={value} value={value}>{label}</MenuItem>)}</Select></FormControl><FormControl fullWidth><InputLabel>{t('navigation.companies')}</InputLabel><Select value={companyId} label={t('navigation.companies')} onChange={event => setCompanyId(event.target.value)}><MenuItem value="all">{t('reports.allCompanies')}</MenuItem>{companies.map(company => <MenuItem key={company.id} value={company.id}>{company.name || company.companyCode || t('common.notSpecified')}</MenuItem>)}</Select></FormControl></Stack><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.25} mt={2.5}><Button variant="contained" startIcon={<PictureAsPdfOutlined />} onClick={() => printPdf(report)}>{t('reports.pdf')}</Button><Button variant="outlined" startIcon={<TableChartOutlined />} onClick={() => exportExcel(report)}>{t('reports.excel')}</Button><Button variant="outlined" startIcon={<DownloadOutlined />} onClick={() => exportCsv(report)}>{t('reports.csv')}</Button></Stack></Paper><Alert severity="info">{t('reports.printHelp')}</Alert>{report.rows.length ? <Paper sx={{ p: 2.5, border: '1px solid #293342' }}><Typography fontWeight={700}>{report.title}</Typography><Typography variant="body2" color="text.secondary" mt={0.5}>{t('reports.ready', { count: report.rows.length })}</Typography></Paper> : <EmptyState title="Šiai ataskaitai duomenų dar nėra" description="Pasirinkite kitą įmonę, laikotarpį arba palaukite, kol atsiras naujų įrašų." kind="reports" />}</Stack>;
}
