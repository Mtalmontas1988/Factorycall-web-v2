...
export function RecentCallsPanel({ calls, onOpenCall }: RecentCallsPanelProps) {
  ...
  const timeLabel = (value?: number | string) => { const time = typeof value === 'number' ? value : Number(value ?? 0); if (!time) return t('common.notSpecified'); const minutes = Math.max(0, Math.floor((Date.now() - time) / 60000)); return minutes < 60 ? t('recentCalls.agoMinutes', { count: minutes }) : new Date(time).toLocaleString(locale); };
